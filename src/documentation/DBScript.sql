...
USE `ParcialBonoDB`;
DROP procedure IF EXISTS `ParcialBonoDB`.`processData`;

DELIMITER $$
USE `ParcialBonoDB`$$
CREATE PROCEDURE processData(
	IN actualDatetime datetime,
    IN sensorId INT
)
BEGIN
	DECLARE average DOUBLE;
	IF (SELECT set_average FROM sensor LEFT JOIN sensorType ON sen_set_type = set_type
    WHERE sensorId = sen_id) = 1 THEN
		SET average = (SELECT num/amount FROM (SELECT SUM(his_value) AS "num", COUNT(his_value) AS "amount" FROM ((SELECT * FROM `sensorHistory` LEFT JOIN `sensor` ON his_sen_id = sen_id) AS s
		LEFT JOIN `sensorType` ON sen_set_type LIKE set_type)
        WHERE his_datetime > (SELECT DATE_SUB(actualDatetime, INTERVAL set_hoursNumber HOUR))) AS b);
    ELSE
		SET average = (SELECT his_value AS "value" FROM (SELECT * FROM `sensorHistory` LEFT JOIN `sensor` ON his_sen_id = sen_id) AS s
		LEFT JOIN `sensorType` ON sen_set_type LIKE set_type ORDER BY his_dateTime DESC limit 1);
    END IF;
...
USE `ParcialBonoDB`;
DROP procedure IF EXISTS `ParcialBonoDB`.`getMessage`;
...
BEGIN
	...
	SET minimum = (SELECT minimum FROM(SELECT set_minimum AS "minimum", set_maximum AS "maximum" FROM sensor LEFT JOIN sensorType ON sen_set_type = set_type WHERE id = sen_id) AS b );
    SET maximum = (SELECT maximum FROM(SELECT set_minimum AS "minimum", set_maximum AS "maximum" FROM sensor LEFT JOIN sensorType ON sen_set_type = set_type WHERE id = sen_id) AS b );
    
    IF newValue <= minimum THEN
		SELECT 2 AS "message";
	ELSEIF newValue > minimum AND newValue <maximum THEN
		SELECT 3 AS "message";
	ELSEIF newValue >= maximum THEN
		SELECT 4 AS "message";
    END IF;
END$$
